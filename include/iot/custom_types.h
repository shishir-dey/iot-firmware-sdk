#ifndef IOT_CUSTOM_TYPES_H
#define IOT_CUSTOM_TYPES_H

#include <stdbool.h>
#include <stdint.h>

typedef bool bb_bool_t;
typedef float bb_float32_t;
typedef double bb_float64_t;
typedef int8_t bb_int8_t;
typedef int16_t bb_int16_t;
typedef int32_t bb_int32_t;
typedef int64_t bb_int64_t;
typedef uint8_t bb_uint8_t;
typedef uint16_t bb_uint16_t;
typedef uint32_t bb_uint32_t;
typedef uint64_t bb_uint64_t;
typedef char* bb_string_t;
typedef uint64_t bb_datetime64_t;
typedef uint64_t bb_timestamp_t;
typedef uint64_t bb_timestamp_ms_t;
typedef uint64_t bb_timestamp_us_t;

#endif /* IOT_CUSTOM_TYPES_H */
